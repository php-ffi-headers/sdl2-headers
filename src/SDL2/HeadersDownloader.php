...
final class HeadersDownloader
{
    ...
    {
        return \vsprintf('https://github.com/libsdl-org/SDL/archive/refs/tags/release-%s.zip', [
            $version->toString(),
        ]);
    }
    ...
    {
        return \sys_get_temp_dir() . '/sdl2-headers-' . $version->toString() . '.zip';
    }
    ...
    public static function download(VersionInterface $version, string $directory): void
    {
        ...
        foreach (self::readArchive($version) as $file) {
            if ($file->isFile() && \str_contains($file->getPathname(), 'include') && \str_ends_with($file->getPathname(), '.h')) {
